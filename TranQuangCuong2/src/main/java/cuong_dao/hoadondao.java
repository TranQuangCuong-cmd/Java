...
//				damua
	...
	public ArrayList<lichsubean> getlichsu(int fl, long makh) {
		try {
			...
			//B2: lay du lieu ve
			String sql="select hoadon.NgayMua , ChiTietHoaDon.madd, didong.tendd, ChiTietHoaDon.SoLuongMua, didong.gia, (didong.gia*ChiTietHoaDon.SoLuongMua) as 'thanhtien' from hoadon inner join ChiTietHoaDon on hoadon.MaHoaDon = ChiTietHoaDon.MaHoaDon inner join didong on ChiTietHoaDon.madd = didong.madd where ChiTietHoaDon.damua = ? and makh = ? order by hoadon.NgayMua desc";
			PreparedStatement cmd = kn.cn.prepareStatement(sql);
			cmd.setInt(1,fl);
			cmd.setLong(2, makh);
			ResultSet rs = cmd.executeQuery();
			...
			int gia = rs.getInt("gia"); 
				...
				ls.add(new lichsubean(ngayMua, madd, tendd, soLuongMua, gia, thanhtien));
			}
